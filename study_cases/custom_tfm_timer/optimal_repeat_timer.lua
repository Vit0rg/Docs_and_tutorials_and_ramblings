-- tfm_lua51_optimal_repeat_timer.lua
-- Strictly O(1), strictly forward, strictly Lua 5.1.
-- Uses repeat..break as an algorithmic early-exit (zero OP_CALL overhead, flat nesting).
local unpack = unpack
...
-- =====================================================================
-- ALGORITHMIC EARLY-EXIT HOT PATH (Flat nesting, zero OP_CALL)
-- =====================================================================
function M.timersLoop()
	for i = 1, #activeLabels do
		local label = activeLabels[i]

		-- Short-circuit flattening
		local t = label and timers[label]
		...
		elseif label then
			-- Stale reference cleanup (External mutation mitigation)
			activeLabels[i] = nil
			freeIndices[#freeIndices + 1] = i
		end
	end
